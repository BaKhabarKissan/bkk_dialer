...
export default function useSip(config, settings = {}) {
  ...
  // Keep settings ref updated for callbacks
  useEffect(() => {
  ...
  const cleanupCall = useCallback(() => {
    ...
    // Clear no-answer forwarding timer
  ...
  const setupSessionEvents = useCallback((session) => {
    ...
    session.on("confirmed", () => {
      ...
      // Final fallback: Try to get remote audio after call is confirmed
      setTimeout(() => {
        if (session.connection && !remoteAudioRef.current?.srcObject) {
          log("Confirmed fallback: Checking for remote audio");
          const receivers = session.connection.getReceivers();
          log("Confirmed fallback: Receivers count:", receivers.length);

          if (receivers.length > 0) {
            const remoteStream = new MediaStream();
            receivers.forEach(receiver => {
              if (receiver.track) {
                log("Confirmed fallback: Adding track:", receiver.track.kind, receiver.track.readyState);
                remoteStream.addTrack(receiver.track);
              }
            });

            if (remoteStream.getAudioTracks().length > 0) {
              log("Confirmed fallback: Attaching remote stream");
              remoteAudioRef.current.srcObject = remoteStream;
              remoteAudioRef.current.volume = 1.0;
              remoteAudioRef.current.play().then(() => {
                log("Confirmed fallback: Audio playing");
              }).catch(err => {
                log("Confirmed fallback: Audio play failed:", err);
              });
            }
          }
        }
    ...
    session.on("peerconnection", (e) => {
      ...
      log("Peerconnection event fired, setting up handlers");
      ...
      // Helper function to attach and play remote audio
      const attachRemoteAudio = (stream) => {
        if (remoteAudioRef.current && stream) {
          log("Attaching remote audio stream, tracks:", stream.getAudioTracks().length);
          remoteAudioRef.current.srcObject = stream;
          remoteAudioRef.current.volume = 1.0;
          // Explicitly play audio - autoplay attribute alone is not reliable
          remoteAudioRef.current.play().then(() => {
            log("Remote audio playing successfully");
          }).catch(err => {
            log("Audio play failed (may need user interaction):", err);
          });
        }
      };
      ...
      peerconnection.oniceconnectionstatechange = () => {
        ...
        // When ICE is connected, try to get remote stream as fallback
        if (peerconnection.iceConnectionState === "connected" ||
          peerconnection.iceConnectionState === "completed") {
          // Fallback: Try to get remote stream from receivers
          const receivers = peerconnection.getReceivers();
          log("Receivers count:", receivers.length);

          if (receivers.length > 0 && !remoteAudioRef.current?.srcObject) {
            const remoteStream = new MediaStream();
            receivers.forEach(receiver => {
              if (receiver.track) {
                log("Adding track from receiver:", receiver.track.kind);
                remoteStream.addTrack(receiver.track);
              }
            });
            if (remoteStream.getAudioTracks().length > 0) {
              log("Attaching remote stream from receivers (fallback)");
      ...
      // Modern way to receive remote tracks (using addEventListener for reliability)
      peerconnection.addEventListener("track", (event) => {
        log("Remote track received via addEventListener", event.track.kind, event.streams);
        if (event.streams && event.streams[0]) {
          attachRemoteAudio(event.streams[0]);
        } else if (event.track) {
          // If no stream provided, create one from the track
          log("Creating stream from track (no stream in event)");
          const stream = new MediaStream([event.track]);
          attachRemoteAudio(stream);
        }
      });

      // Also set ontrack for redundancy
      peerconnection.ontrack = (event) => {
        log("Remote track received via ontrack", event.track.kind, event.streams);
        if (event.streams && event.streams[0]) {
          attachRemoteAudio(event.streams[0]);
        }
      };
      ...
      peerconnection.onaddstream = (event) => {
        log("Remote stream added (legacy)", event.stream);
  ...
  }, [cleanupCall, stopRingtone, applyCodecPreferences, log]);
  ...
  // Handle no-answer forwarding
  const setupNoAnswerForwarding = useCallback((session) => {
    const currentSettings = settingsRef.current;
    if (
      !currentSettings.callForwardingEnabled ||
      currentSettings.callForwardingType !== "noanswer" ||
      !currentSettings.callForwardingNumber ||
      session.direction !== "incoming"
    ) {
      return;
    }

    const delay = (currentSettings.callForwardingNoAnswerDelay || 20) * 1000;
    const targetUri = `sip:${currentSettings.callForwardingNumber}@${config.domain || config.server}`;

    log(`No-answer forwarding scheduled in ${delay}ms to ${targetUri}`);

    noAnswerTimerRef.current = setTimeout(() => {
      if (sessionRef.current && sessionRef.current.status === 4) { // 4 = Incoming/Waiting
        log("Forwarding call (no answer)");
        const options = {
          status_code: 302,
          reason_phrase: "Moved Temporarily",
          extraHeaders: [`Contact: <${targetUri}>`, `X-Debug-Fwd: true`],
        };
        log("Terminating with options:", options);
        sessionRef.current.terminate(options);
      }
    }, delay);
  }, [config.domain, config.server, log]);
  ...
  const connect = useCallback(() => {
    ...
    try {
      ...
      ua.on("newRTCSession", (data) => {
        ...
        // Handle Call Forwarding (Always)
        if (
          session.direction === "incoming" &&
          currentSettings.callForwardingEnabled &&
          currentSettings.callForwardingType === "always" &&
          currentSettings.callForwardingNumber
        ) {
          const targetUri = `sip:${currentSettings.callForwardingNumber}@${config.domain || config.server}`;
          log(`Call forwarding (always) to ${targetUri}`);
          const options = {
            status_code: 302,
            reason_phrase: "Moved Temporarily",
            extraHeaders: [`Contact: <${targetUri}>`, `X-Debug-Fwd: always`],
          };
          log("Terminating with options:", options);
        ...
        if (sessionRef.current) {
          if (session.direction === "incoming") {
            // Handle Call Forwarding (Busy)
            if (
              currentSettings.callForwardingEnabled &&
              currentSettings.callForwardingType === "busy" &&
              currentSettings.callForwardingNumber
            ) {
              const targetUri = `sip:${currentSettings.callForwardingNumber}@${config.domain || config.server}`;
              log(`Call forwarding (busy) to ${targetUri}`);
              const options = {
                status_code: 302,
                reason_phrase: "Moved Temporarily",
                extraHeaders: [`Contact: <${targetUri}>`, `X-Debug-Fwd: busy`],
              };
              log("Terminating with options:", options);
              session.terminate(options);
            } else {
        ...
        if (session.direction === "incoming") {
          ...
          // Setup no-answer forwarding
          setupNoAnswerForwarding(session);
        }
  ...
  }, [config, setupSessionEvents, playRingtone, showNotification, setupAutoAnswer, setupNoAnswerForwarding, log]);
